...
exports.attach = function (options) {
  ...
  // 
  createKnoten = function (networkid, leknoten, callback) {
    ...
    app.log.debug(networkid + ": REG: Create Knoten!", leknoten);
    
    // get the network
    Network.get(networkid, function(err, network) {
      ...
      if (err) {
        ...
        network.createKnoten(leknoten, function (err, result) {
          ...
          if (err) {
            ...
            // success!
            app.log.debug(networkid + ": REG: Success!", a.result);
            callback(null, a);
  ...
  updateKnoten = function (networkid, leknoten, callback) {
    
    // this always get called with given:
    // networkid, registered number, pass, maybe mac 
           
    // also, all prechecks are already done!
    // if we reach this point, we are updating a registered number (also means pass was checked).

    app.log.debug(networkid + ": REG: Update Knoten:", leknoten);
    ...
    // get the `Knoten`
    Knoten.get('network/' + networkid + '/' + leknoten.id, function (err, knoten) {
      ...
      if (err) {
        ...
        knoten.update(leknoten, function(err, result) {
          ...
          if (err) {
            ...
            // success!
            app.log.debug(networkid + ": HEARTBEAT: ", a.result);
            callback(null, a);
  ...
  app.register.getAll = function (networkid, wanted, callback) {
   ...
   // Get the network
    Network.get(networkid, function(err, network) {
      ...
      if (err || !network) {
        var msg = "Network " + networkid + " not found!";
      ...
      network.knotens(function (err, res) {
        ...
        // If there was no error, 
          ...
          // If no error, we build an answer:
  ...
  app.register.get = function (networkid, number, callback) {
     ...
     Network.get(networkid, function(err, network) {
      ...
      network.getKnoten(number, function(err, knoten) {
        ...
        if (err || !knoten) {
          ...
          var msg = "No knoten " + number + " found in network " + networkid;
          return app.fail(404, msg, callback);
  ...
  app.register.create = function (networkid, mac, pass, callback) {
    ...
    // Get the network
     Network.get(given.networkid, function(err, network) {
      
       // fail if there was an error
       if (err || !network) {
         var msg = "Network " + given.networkid + " not found!";
         return app.fail(404, msg, callback);
       };    
    
      // Let's check if mac is already registered in the network:
      // 
      // TODO: use Knoten.find({'network_id': given.networkid, 'mac': given.mac}, function(err, knoten) {};)
      // 
  ...
  app.register.update = function (networkid, number, mac, pass, callback) {    
    ...
    // app.log.warn('network/' + networkid + '/' + number);
    Knoten.get('network/' + given.networkid + '/' + given.number, function(err, registered) {
      ...
      if (!err) {
        ...
        if (registered.pass) {
          ...
          if (given.pass !== registered.pass) {
            ...
            updateKnoten(given.networkid, given, callback);
      ...
      createKnoten(given.networkid, given, callback);
