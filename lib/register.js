...
exports.attach = function (options) {
  ...
  writeKnoten = function (leknoten, callback) {
    ...
    Knoten.find({ "number": leknoten.number}, function (err, knoten) {
      
      if (err) {
        
        app.fail(500, null, callback);
        return;
        
      } else {
        
        app.log.warn("found knoten:", knoten);
        
        if (knoten.toString() === "") { // TODO: better check
          
          // the number does not exist. CREATE!
          app.log.debug('Create Knoten!', knoten);
        ...
        else if (knoten.toString() !== "") { // TODO: better check
          
          // the number does exist. UPDATE!
          app.log.debug('Update Knoten!', knoten);
          
          // timestamp the knoten
          // knoten.last_seen = new Date().getTime();
          
          knoten.update(function(err, result) {
