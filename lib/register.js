...
exports.attach = function (options) {
  ...
  var leaseTimedOut = function (knoten, network) {
    ...
    var networkLease = tool.daysToMilliseconds(network.lease_days || 30);
  ...
  // ## deleteKnoten
  deleteKnoten = function (network_id, number, callback) {
    
    // this is only called internally when a lease timed out
    
    // destroy the `Knoten`
    Knoten.destroy('network/' + network_id + '/' + number, function (err, result) {
      ...
      // > and it should be "204" (successfully deleted).
      else if (result.status !== 204) {
        app.log.error(network_id + ": could not delete: ", number, err, result);
        return app.fail(500, JSON.stringify(err), callback);
      }

      // - deleted knoten!
      app.log.debug(network_id + ": DELETED: ", number);
      callback(null);
