...
exports.attach = function (options) {
  ...
  app.register.getAll = function (properties, callback) {
    ...
    // default properties, if none given
    if (!properties) {
      properties = ["knoten", "numbers", "macs"];
    }
    ...
    db.all(function(err, res) {
      ...
      // if no error, 
        ...
        // - set up an array for each requested property
        ...
        Object.keys(res).forEach(function(nr){
          ...
          if (mac) {
            ...
            // add it to current knoten and 
            knoten.mac = mac;
            
            // add its mac property to macs array (as string).
            a.result.macs.push(mac);
            
          }
          
          // then add number property of knoten to numbers array (as integer) 
          a.result.numbers.push(knoten.number);
          
          // and the knoten itself into the knoten array (as an object).
