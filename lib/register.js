...
exports.attach = function (options) {
  ...
  // 
  // ## leaseTimedOut
  var leaseTimedOut = function (knoten, network) {
    
    // - get *configured lease* of network, convert to milliseconds
    var networkLease = (network.lease_seconds? network.lease_seconds*1000 : tool.daysToMilliseconds(network.lease_days || 30));
    
    // - calculate *current lease* of knoten in milliseconds
    var knotenLease = (new Date() - new Date(knoten["last_seen"]));

    return (knotenLease > networkLease)
  }
    ...
    network.knotens(function (err, res) {
      ...
      checkNumber: while (!freeNumber || tryNumber < 255 ) {
        ...
        if (register.hasOwnProperty(tryNumber)) {
          ...
          if (leaseTimedOut(register[tryNumber], network)) {

            // - if *lease* timed out, we have our *number*!
            app.log.debug("found number because lease timed out: "+tryNumber);
        ...
        // continue search with incremented tryNumber
      ...
      // - if we finished the loop without finding a *number*,
      // we give up `:(`
      if (!freeNumber) {
        callback(new Error("Search exhausted!"));
      } else {
  ...
  createKnoten = function (network_id, leknoten, callback) {
    ...
    //  * *network_id*
    //  * "free" *number* – fresh or timed out lease
    //  * *pass*
    //  * maybe *mac*
    
    // We still need to check if lease timed out -> deleteKnoten
    ...
    Network.get(network_id, function(err, network) {
      ...
      if (err) {
        ...
        return app.fail(500, err, callback);
      }
      
      async.waterfall([
        function checkLeaseAndDelete(callback) {
          // try to get the existing knoten with this number
          Knoten.get('network/' + network_id + '/' + leknoten.id, function (err, knoten) {

            // if something was found and if the lease timed out…
            if (!err && leaseTimedOut(knoten, network)) {
              deleteKnoten(knoten, network, function (err) {
                if (err) { return callback(err); }
                app.log.debug("Deleted Knoten "+ knoten.id +"!")
                return callback(null);
              });
            } else {
              // if the lease did not time out, continue creation.
              return callback(null);
          }
          });
        }
        ...
        if (err) { return app.fail(500, err, callback); }
    ...
    Knoten.get('network/' + network_id + '/' + leknoten.id, function (err, knoten) {
      ...
      if (err) {
        ...
        knoten.update(leknoten, function(err, result) {
          ...
          if (err) {
            // - db error at this stage is our fault
            return app.fail(500, JSON.stringify(err), callback);
          } 
  ...
  // ## deleteKnoten
  deleteKnoten = function (knoten, network, callback) {
    
    // this is only called internally when a lease timed out
    app.log.debug(network.id + ": Deleting… : ", knoten.id);

    // destroy the `Knoten`
    Knoten.destroy(knoten.id, function (err, result) {
      
      if (err) {
        return callback(new Error(network.id + ": could not delete: "+knoten.id+"! ("+err+")"));
      }

      // - deleted knoten!
      app.log.debug(network.id + ": DELETED: ", knoten.id);
      return callback(null);
  ...
  app.register.create = function (network_id, mac, pass, callback) {
    ...
    if (mac) {
      if (!tool.isValidMac(mac)) {
        return app.fail(400, { error: "Malformed MAC!" }, callback);
      } else { 
      ...
      network.knotens(function (err, registered) {
        ...
        if (!ok) {
          ...
          getfirstFreeNumber(network, function (err, number) {
            ...
            if (err) {
              return app.fail(404, "No free numbers!", callback);
            } 
            
            given.number = number;
            
            app.log.debug('Creating Knoten…', given);
            createKnoten(network.id, given, callback);
