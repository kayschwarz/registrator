...
Knoten.property('number', 'number', {
  ...
  get: function() {
    ...
    return parseInt(id.toString().replace(reg, "$1"), 10);
  }
});
...
exports.init = function (done) {
  ...
  var app = this,
      couchdb = app.config.get('couch');
  
  // set up resourceful to use couchdb if configured, 
  if (couchdb) {
    app.log.debug(["Storage: Using couchdb ",couchdb.uri,':',couchdb.port].join(''))
    resourceful.use('couchdb', couchdb);
  } else {
    app.log.warn("Storage: Using in-memory storage. Will loose all data on exit!")
